        ...
        script {
          docker.image(BUILD_IMAGE).inside("-v /var/run/docker.sock:/var/run/docker.sock") {
            sh """
              ...
              tar -zcvf mithril.tar.gz bookinfo spire istio deploy-all.sh cleanup-all.sh forward-port.sh create-kind-cluster.sh create-docker-registry-secret.sh doc/poc-instructions.md
