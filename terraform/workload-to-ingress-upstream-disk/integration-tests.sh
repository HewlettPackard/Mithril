...
kubectl create ns spire && TAG=latest HUB=${hub} ./deploy-all.sh &&
...
HUB=${hub} AWS_ACCESS_KEY_ID=${access_key} AWS_SECRET_ACCESS_KEY=${secret_access_key} ./create-docker-registry-secret.sh &&
TAG=latest HUB=${hub} && ./deploy-all.sh &&
...
kubectl exec -i -t pod/$CLIENT_POD -c sleep -- /bin/sh -c "cat -e /tmp/response_productpage.txt > /tmp/response_productpage.txt" && cd /mithril/e2e && touch ${build_tag}-${usecase}-result.txt && go test -v e2e -run TestWorkloadToIngressUpstreamDisk 2>&1 | tee ${build_tag}-${usecase}-result.txt &&
