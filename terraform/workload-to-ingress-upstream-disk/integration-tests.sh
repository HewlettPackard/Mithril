...
bash -c 'cd /mithril/usecases && find . -type f -iname "*.sh" -exec chmod +x {} \; && cd workload-to-ingress-upstream-disk/server-cluster && ../../../POC/create-kind-cluster.sh &&
...
kubectl exec -i -t pod/$CLIENT_POD -c sleep -- /bin/sh -c "curl -sSLv http://istio-ingressgateway.istio-system.svc:8000/status/200 2>&1 | tee /tmp/workload_to_ingress_upstream_disk_test_response.txt" &&
