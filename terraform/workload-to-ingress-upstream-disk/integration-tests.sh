...
kubectl exec -i -t pod/$CLIENT_POD -c sleep -- /bin/sh -c "curl -sSLk http://app.example.org:8000/status/200 2>&1 | tee /tmp/workload_to_ingress_upstream_disk_test_response.txt" &&
